...
struct RingColors {
    ...
    var colors: [UIColor] = [UIColor.green.withAlphaComponent(0.2), UIColor.yellow.withAlphaComponent(0.2), UIColor.red.withAlphaComponent(0.2)]// Must be 3 of these
...
func displayInfo(ring:RingColors) -> String {
    ...
    str += "Pattern Altitude: \(Int(ring.patternAltitude))'\n"
    str += "Elevation: \(Int(ring.elevation))'\n"
    str += "Safety % : >=\(Int(ring.safety[1]*100))%, \(Int(ring.safety[2]*100))%-\(Int(ring.safety[3]*100))%, \(Int(ring.safety[4]*100))%-\(Int(ring.safety[5]*100))%\n"
